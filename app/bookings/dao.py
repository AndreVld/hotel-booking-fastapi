...
class BookingDAO(BaseDAO):
    ...
    async def add(
        ...
        rooms_left = await cls.get_num_of_available_rooms(room_id=room_id, date_from=date_from, date_to=date_to)
    ...
    async def delete(cls, booking_id, user_id: int):
        async with async_session() as session:
            await session.execute(
                delete(cls.model).where(and_(cls.model.id == booking_id, cls.model.user_id == user_id))
            )
            await session.commit()
